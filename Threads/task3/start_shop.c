...
int left_buyers = BUYERS;

int create_shop(struct product products_list[PRODUCTS]) {
  if (BUYERS <= 0 || PROVIDERS <= 0 || PRODUCTS <= 0) {
    printf("Число покупателей, поставщиков и продуктов не может быть меньше "
           "либо равно 0!\n");
    return -1;
  }
  ...
  if (pthread_create(&providers_thread, NULL, start_provider,
                     (void *)products_list) != 0) {
    perror("Ошибки при создании потока!");
    return -1;
  }
